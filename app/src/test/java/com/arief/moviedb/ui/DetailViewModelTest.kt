...
class DetailViewModelTest {
    ...
    fun setup(){
        ...
        runBlocking {
            whenever(apiMovieDBRepo.getSimilar(1, BuildConfig.API_KEY, 1)).thenReturn(
                result1
            )
            whenever(apiMovieDBRepo.getSimilar(2, BuildConfig.API_KEY, 1)).thenReturn(
                result2
            )
            whenever(networkHelper.isNetworkConnected()).thenReturn(true)
        }
        viewModel = DetailViewModel(apiMovieDBRepo, networkHelper)
    }

    @Test
    fun `when getSimilar called then observe with live data`() = runBlocking {
        viewModel.movies.observeForever(moviesObserver)

        viewModel.getSimilar(1)
        verify(apiMovieDBRepo).getSimilar(1, BuildConfig.API_KEY, 1)
    ...
    fun `when getNowPlaying is called twice then load more data`() = runBlocking {
        ...
        viewModel.getSimilar(2)
        verify(moviesObserver, atLeastOnce()).onChanged(Resource.loading(null))
        verify(apiMovieDBRepo).getSimilar(2, BuildConfig.API_KEY, 1)
