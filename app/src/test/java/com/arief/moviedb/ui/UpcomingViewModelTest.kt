...
class UpcomingViewModelTest {
    ...
    fun setup(){
        ...
        runBlocking {
            whenever(apiMovieDBRepo.getUpcoming(BuildConfig.API_KEY, 1)).thenReturn(
                result1
            )
            whenever(apiMovieDBRepo.getUpcoming(BuildConfig.API_KEY, 2)).thenReturn(
                result2
            )
            whenever(networkHelper.isNetworkConnected()).thenReturn(true)
        }
        viewModel = UpcomingViewModel(apiMovieDBRepo, networkHelper)
    }
    ...
    fun `when getNowPlaying called then observe with live data`() = runBlocking {
        ...
        verify(apiMovieDBRepo).getUpcoming(BuildConfig.API_KEY, 1)
    ...
    fun `when getNowPlaying is called twice then load more data`() = runBlocking {
        ...
        verify(apiMovieDBRepo).getUpcoming(BuildConfig.API_KEY, 2)
